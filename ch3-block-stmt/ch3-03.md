...
词法解析的输入是字节流，在解析的过程中常常需要向前多看一个字符，同时也可能要忽略一些字符，为此我们需要先构造一个字节流的Stream对象（这个设计同样可以复制到Token流Stream，用于语法解析）。
...
NewStream根据文件名和内容构造一个Stream对象。其Name和Input方法分别返回源文件的名字和内容。Pos方法返回当前读的位置，Peek预取下一个字符、Read读取下一个字符、Unread用于一次回退。Accept会尝试消费一个存在于valid中的字符，而AcceptRun则会尝试尽量消费多个存在于valid中的字符。EmitToken方法用于产生当前读取到的记号面值和位置信息，IgnoreToken则忽略当前的记号。

Stream中最基础的核心函数Read和Unread实现如下：
...
func NewLexer(name, input string) *Lexer {
	p := &Lexer{ Stream: NewStream(name, input)}
	p.run()
	return p
}
...
手写通过`p.EmitToken()`调用内部Stream对象提供的方法，然后构造新的token.Token对象并添加到p.tokens列表。如果是标识符类型，则通过token.Lookup(lit)查询是否为关键字类型。
...
14: RBRACE      : "}"                  // ../hello.ugo:5:1
15: EOF         : ""                   // ../hello.ugo:5:3
----
